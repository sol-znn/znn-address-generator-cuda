// sha256.h -- minimal host-side SHA-256. The generator computes its BIP-39
// checksums on the GPU; this copy is used only by --selftest, to confirm the
// test vector really is the mnemonic for the entropy it was unpacked from.
#ifndef SHA256_H
#define SHA256_H
...
// sha256 hashes `len` bytes of `msg`, writing 32 bytes to `out`.
inline void sha256(const uint8_t* msg, size_t len, uint8_t out[32]) {
    ...
    uint8_t buf[256];  // enough for the small entropy inputs used here
    memset(buf, 0, padded);
    memcpy(buf, msg, len);
    buf[len] = 0x80;
